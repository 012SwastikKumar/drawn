...
export default function App() {
          ...
          switch (msg.type) {
            ...
            case 'kicked': {
              ...
              if (typeof window !== 'undefined') {
                ...
                window.sessionStorage.removeItem('drawn_username');
                window.sessionStorage.removeItem('drawn_avatar');
              }
              alert(reason || 'You have been kicked from the room by the host.');
  ...
  // Host Action: Transfer Host Privilege
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col overflow-hidden" id="app-root">
      ...
      {!room ? (
        ...
        <div className="flex flex-col h-screen h-[100dvh] overflow-hidden bg-slate-50 text-slate-800 animate-fade-in" id="active-game-view">
          ...
          <header className="h-16 flex-none bg-white border-b border-slate-200 px-3 sm:px-6 flex items-center justify-between shadow-xs" id="game-header-bar">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex border-l border-slate-200 pl-3 h-8 flex-col justify-center">
                <p className="text-[10px] text-indigo-600 font-black uppercase tracking-wider leading-none mb-1">
                  Round {room.round} / {room.maxRounds}
                </p>
                <p className="text-[9px] text-slate-400 font-black uppercase tracking-wider leading-none">
                  Room #{roomId} • {Object.keys(room.players).length} Players
                </p>
              </div>
            </div>
              ...
              <div className="relative" id="header-options-dropdown">
                ...
                {showMobileMenu && (
                  <>
                    ...
                    <div className="absolute right-0 mt-1.5 w-48 bg-white border border-slate-200/90 rounded-2xl p-1.5 shadow-xl z-50 animate-fade-in text-slate-800 flex flex-col gap-0.5">
                      {player?.isHost && (
                        <button
                          onClick={() => {
                          ...
                          className="w-full px-3 py-2 bg-amber-50/50 hover:bg-amber-50 rounded-xl text-left text-xs font-bold text-amber-700 flex items-center gap-2 cursor-pointer transition-all border border-amber-100/50"
                        >
                          <Crown className="w-4 h-4 text-amber-600 animate-pulse" />
                          <span>Host Access Panel</span>
                        </button>
                      )}
                      ...
                      {player?.isHost && room.status !== 'LOBBY' && (
                        <button
                          ...
                          className="w-full px-3 py-2 bg-indigo-50/50 hover:bg-indigo-50 rounded-xl text-left text-xs font-bold text-indigo-700 flex items-center gap-2 cursor-pointer transition-all"
                        >
                      ...
                      <button
                        ...
                        className="w-full px-3 py-2 hover:bg-slate-50 rounded-xl text-left text-xs font-bold text-slate-750 flex items-center gap-2 cursor-pointer transition-all"
                      >
                      ...
                      <button
                        onClick={toggleSound}
                        className="w-full px-3 py-2 hover:bg-slate-50 rounded-xl text-left text-xs font-bold text-slate-750 flex items-center justify-between cursor-pointer transition-all"
                      >
                        <div className="flex items-center gap-2">
                          {soundEnabled ? <Volume2 className="w-4 h-4 text-emerald-600" /> : <VolumeX className="w-4 h-4 text-slate-400" />}
                          <span>Game Sounds</span>
                        </div>
                        <span className={`text-[9px] font-black uppercase ${soundEnabled ? 'text-emerald-600' : 'text-slate-450'}`}>
                          {soundEnabled ? 'ON' : 'OFF'}
                        </span>
                      </button>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          {copiedLink ? <Check className="w-4 h-4 text-emerald-600 animate-pulse" /> : <Copy className="w-4 h-4 text-indigo-500" />}
                          <span>Copy Invite Link</span>
                        </div>
                        {copiedLink && <span className="text-[9px] font-black text-emerald-600 uppercase">Copied</span>}
                      </button>
                      ...
                      <button
                        ...
                        className="w-full px-3 py-2 hover:bg-red-50 rounded-xl text-left text-xs font-black text-red-650 flex items-center gap-2 cursor-pointer transition-all"
                      >
          ...
          <main className="flex-1 flex flex-col overflow-hidden p-2 sm:p-2.5 gap-2.5 bg-slate-50 relative animate-fade-in" id="game-arena-body">
            ...
            <section className={`${isMobile && activeMobileTab !== 'chats' ? 'hidden' : 'flex'} flex-col bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden min-h-0 flex-1 sm:flex-none sm:w-80`} id="right-chat-section">
              ...
              <div className="hidden sm:block bg-slate-50 border-b border-slate-200/80 p-3 select-none" id="chat-sidebar-leaderboard">
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between p-2 rounded-xl text-xs border transition-all ${
                            isSelf
                              ? 'bg-indigo-50/65 border-indigo-200 font-extrabold shadow-3xs'
        ...
        {isMobile && (
          ...
          {/* Host Access Control Modal (Kick, transfer host, end game) */}
          {showHostPanel && (
            <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in" id="host-panel-modal-overlay">
              <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full shadow-2xl relative text-slate-800" id="host-panel-modal">
                <h3 className="text-sm font-black uppercase tracking-widest text-slate-850 flex items-center gap-2 mb-4">
                  <Crown className="w-5 h-5 text-amber-500 animate-pulse" />
                  <span>Host Access Control</span>
                </h3>
                
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Manage Players & Game State</p>

                <div className="space-y-4 max-h-[300px] overflow-y-auto mb-6 pr-1" id="host-panel-modal-players-list">
                  <span className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-2">Room Players</span>
                  {(Object.values(room.players) as Player[]).filter((p) => p.id !== playerId).length === 0 ? (
                    <div className="text-center py-4 bg-slate-50 border border-slate-150 rounded-xl text-xs font-semibold text-slate-450">
                    ...
                    (Object.values(room.players) as Player[])
                      ...
                      .map((p) => (
                        <div key={p.id} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-150 rounded-xl gap-3">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="w-2.5 h-2.5 rounded-full shrink-0 shadow-3xs border border-white/20" style={{ backgroundColor: p.color }} />
                            <span className="text-xs font-extrabold text-slate-800 truncate">{p.name}</span>
                            {p.disconnected && (
                              <span className="text-[8px] bg-red-100 text-red-700 font-bold px-1 py-0.5 rounded-md uppercase">Offline</span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <button
                              onClick={() => {
                                if (confirm(`Are you sure you want to transfer host permissions to ${p.name}?`)) {
                                  handleTransferHost(p.id);
                                  setShowHostPanel(false);
                                }
                              }}
                              className="px-2.5 py-1 bg-white hover:bg-amber-50 text-amber-700 border border-slate-200 hover:border-amber-200 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer shadow-3xs"
                            ...
                            <button
                              onClick={() => {
                                  if (confirm(`Are you sure you want to kick ${p.name} from the room?`)) {
                                    handleKickPlayer(p.id);
                                  }
                              }}
                              className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-650 border border-red-200 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer"
                              title="Kick Player"
                            >
                              Kick
                            </button>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-100">
                  {/* End game early logic */}
                  {room.status !== 'LOBBY' && room.status !== 'GAME_OVER' && (
                    <button
                      onClick={() => {
                        if (confirm('Are you sure you want to end this game early? Final rankings will be determined immediately.')) {
                          handleForceEndGame();
                          setShowHostPanel(false);
                        }
                      }}
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 font-bold text-xs uppercase tracking-wider rounded-xl transition-colors cursor-pointer shadow-xs text-center"
                  >
                    Close Panel
                  </button>
                </div>
